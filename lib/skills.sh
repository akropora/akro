...
skill_run_tool() {
    ...
    AKRO_ROOT="$AKRO_ROOT" AKRO_DATA_DIR="$AKRO_DATA_DIR" AKRO_GLOBAL_DIR="$AKRO_GLOBAL_DIR" AKRO_PROJECTS_DIR="$AKRO_PROJECTS_DIR" AKRO_STATE_FILE="$AKRO_STATE_FILE" AKRO_RUNTIME_DIR="$AKRO_RUNTIME_DIR" AKRO_WORKSPACE="$AKRO_WORKSPACE" CURRENT_PROJECT_DIR="$CURRENT_PROJECT_DIR" CURRENT_PROJECT_SLUG="$CURRENT_PROJECT_SLUG" CURRENT_PROJECT_NAME="$CURRENT_PROJECT_NAME" AKRO_SKILL_ARG="$arg" TAVILY_API_KEY="$TAVILY_API_KEY" TAVILY_API_URL="$TAVILY_API_URL" TAVILY_MAX_RESULTS="$TAVILY_MAX_RESULTS" DOCUMENT_MAX_BYTES="$DOCUMENT_MAX_BYTES" DOCUMENT_INLINE_BYTES="$DOCUMENT_INLINE_BYTES" OLLAMA_API_BASE="$OLLAMA_API_BASE" OLLAMA_CHAT_URL="$OLLAMA_CHAT_URL" OLLAMA_TAGS_URL="$OLLAMA_TAGS_URL" OLLAMA_EMBED_URL="$OLLAMA_EMBED_URL" NEURON_MODEL="$NEURON_MODEL" LIBRARIAN_MODEL="$LIBRARIAN_MODEL" AGENT_MODEL="$AGENT_MODEL" BASE_MODEL="$BASE_MODEL" AKRO_EMBED_MODEL="$AKRO_EMBED_MODEL" AGENT_MAX_STEPS="$AGENT_MAX_STEPS" AGENT_NUM_CTX="$AGENT_NUM_CTX" AGENT_NUM_PREDICT="$AGENT_NUM_PREDICT" AGENT_CONTROLLER_NUM_CTX="$AGENT_CONTROLLER_NUM_CTX" AGENT_CONTROLLER_NUM_PREDICT="$AGENT_CONTROLLER_NUM_PREDICT" AGENT_MAX_READ_CHARS="$AGENT_MAX_READ_CHARS" AGENT_MAX_TOOL_OUTPUT="$AGENT_MAX_TOOL_OUTPUT" AGENT_MAX_WRITE_CHARS="$AGENT_MAX_WRITE_CHARS" AGENT_ACTION_MAX_CHARS="$AGENT_ACTION_MAX_CHARS" AGENT_SHOW_TOOLS="$AGENT_SHOW_TOOLS" AGENT_SHOW_PLAN="$AGENT_SHOW_PLAN" AGENT_SHOW_INSTRUCTIONS="$AGENT_SHOW_INSTRUCTIONS" AGENT_VERBOSE="$AGENT_VERBOSE" AGENT_COMMAND_TIMEOUT="$AGENT_COMMAND_TIMEOUT" "$entry" < "$input" > "$output" 2> "$err" &
    pid=$!
