...
ollama_stream_chat() {
    ...
    local payload="" line="" chunk="" saw_content=0 fifo="" curl_pid=0 curl_rc=0 spinner_pid=0
    local -a cycle=('⠾' '⠽' '⠻' '⠟' '⠯' '⠟' '⠻' '⠽')
    ...
    payload="$(jq -n \
        ...
        '{model:$model,messages:$messages,stream:true,think:false,keep_alive:"10m",options:{num_ctx:$ctx,num_predict:$predict}}')"
    ...
    curl -sS --connect-timeout 10 --max-time 600 \
        ...
        -d "$payload" "$OLLAMA_CHAT_URL" > "$fifo" 2> "$error_file" &
    curl_pid=$!
    exec 3< "$fifo"
